...
public class RecipesEditorComboBoxLogic : BaseNetLogic
{
	public override void Start()
	{
        ...
        task2?.Dispose();
            task2 = new DelayedTask(() => {Refresh_ComboBox() ; }, 500, LogicObject);   //Recipe delay for data acquisition
            task2.Start();
		task?.Dispose();
            task = new DelayedTask(() => {LoadSelectedRecipeData() ; }, 1000, LogicObject);   //Recipe delay for data acquisition
            task.Start();
   ...
   private void Refresh_ComboBox()
	...
	{
		task?.Dispose();
            task = new DelayedTask(() => {LoadSelectedRecipeData() ; }, 200, LogicObject);   //Recipe delay for data acquisition
            task.Start();
	}
