...
public class StringForOPCScreentoScrew : BaseNetLogic
{
    ...
    public string ScreenPath = "UI/Screens/Screen056_Read_STRING_OPC/Layout";
    ...
    public void Method1()
    {
       ...
       /* for (int m=1;m<9;++m)
        ...
        for (int j=1;j<5;j++)
        {
            for (int i = 1; i<26; i++)
            {
                DynamicLinkPath =ScreenPath + $"/Photoeyes/PhotoEyeBarScrewToScreen_type1/Rectangle{j}/Opacity/KeyValueConverter1/Source/ExpressionEvaluator1/Source0/StringFormatter1/Source{i-1}/DynamicLink";
                TextBoxPath =PopUp + $"/STRING_OPC/TypeValueGroup1/TypeValueDispay{j}/Rectangle1/TextBox{i}/Text/DynamicLink";
                Project.Current.GetVariable(TextBoxPath).Value = Project.Current.GetVariable(DynamicLinkPath).Value;

            }
            for (int k = 1; k<26; k++)
            {
                DynamicLinkPath = ScreenPath + $"/Photoeyes/PhotoEyeBarScrewToScreen_type1/Rectangle{j}/FillColor/KeyValueConverter1/Source/ExpressionEvaluator1/Source0/StringFormatter1/Source{k-1}/DynamicLink";
                TextBoxPath = PopUp + $"/STRING_OPC/TypeValueGroup1/TypeValueDispay{j+4}/Rectangle1/TextBox{k}/Text/DynamicLink";
                Project.Current.GetVariable(TextBoxPath).Value = Project.Current.GetVariable(DynamicLinkPath).Value;
